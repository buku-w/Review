...
| 使用clone方法：当前类需要实现Cloneable接口，实现clone()方法 | 没有调用构造函数 |
| 使用反序列化：从文件、网络中获取一个对象的二进制流          | 没有调用构造函数 |
...
<img src="C:\Users\Administrator\AppData\Roaming\Typora\typora-user-images\image-20210215185341657.png" alt="image-20210215185341657"  />
...
执行引擎负责把字节码指令翻译（解释/编译）成机器指令

Java 是半解释半编译型语言。解释器结合PC计数器逐行解释字节码，效率较低，但优点是程序启动后立即执行，响应快，省去编译时间；即时编译器需要把字节码编译成机器码，这个过程需要一定时间，但优点是编译成本地代码之后，执行效率高

![image-20210216154055047](C:\Users\Administrator\AppData\Roaming\Typora\typora-user-images\image-20210216154055047.png)

![image-20210216153837927](C:\Users\Administrator\AppData\Roaming\Typora\typora-user-images\image-20210216153837927.png)



![image-20210216154607789](C:\Users\Administrator\AppData\Roaming\Typora\typora-user-images\image-20210216154607789.png)
...
![image-20210216160816225](C:\Users\Administrator\AppData\Roaming\Typora\typora-user-images\image-20210216160816225.png)

![image-20210216160627301](C:\Users\Administrator\AppData\Roaming\Typora\typora-user-images\image-20210216160627301.png)



![image-20210216162143105](C:\Users\Administrator\AppData\Roaming\Typora\typora-user-images\image-20210216162143105.png)

![image-20210216172215064](C:\Users\Administrator\AppData\Roaming\Typora\typora-user-images\image-20210216172215064.png)

![image-20210216172432503](C:\Users\Administrator\AppData\Roaming\Typora\typora-user-images\image-20210216172432503.png)



**字符串常量池：**

![image-20210217153241989](C:\Users\Administrator\AppData\Roaming\Typora\typora-user-images\image-20210217153241989.png)
...
- 如果不是用双引号声明的 String 对象，可以使用 String 提供的 intern 方法。String.intern() 是一个 Native 方法，它的作用是：如果运行时常量池中已经包含一个等于此 String 对象内容的字符串，则返回常量池中该字符串的引用；如果没有，JDK1.7之前（不包含1.7）的处理方式是在常量池中**创建**与此 String 内容相同的字符串，并返回常量池中创建的字符串的引用，JDK1.7以及之后的处理方式是在常量池中**记录**堆中此字符串的引用，并返回该引用。
...
```java
          ...
          String str3 = "str" + "ing";//常量池中的对象，字符串常量之间用+拼接，在编译成字节码时会直接优化成str3="string"
          String str4 = str1 + str2; //在堆上创建的新的对象，+前后有变量时，则底层会生成StringBuilder，把字符串变量一个个append进去，最后返回toString()。（final String是常量，不是变量）
